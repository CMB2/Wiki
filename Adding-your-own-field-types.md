...
> **Update: This field is now [in CMB2 core](https://github.com/WebDevStudios/CMB2/blob/master/example-functions.php#L72), so consider this tutorial as an example.**
...
* `5` -- This is the number of parameters your custom function will receive. This hook can accept up to 5 parameters:
	...
	* `$field_type_object`: This is an instance of the `CMB2` object and gives you access to all of the methods that CMB2 uses to build its field types.

	We've set this to 5 so that we  can access the `$field_type_object` object. This allows us to use CMB2's built in input method.
...
* `$object_id`: The id of the object you are working with. Most commonly, the post id.
* `$field_object`: `CMB2_Field` object.
* `$sanitizer`: This is an instance of the `CMB2_Sanitize` object and gives you access to all of the methods that CMB2 uses to sanitize its field values.
...
```
Then, in our fields array, we would add the `select` type and pass the `cmb2_get_term_options` function as our 'options' array.
...
...
        'fields' => array(
            array(
                ...
                'id'      => 'featured_theme',
                'type'    => 'select',
                'options' => cmb2_get_term_options( 'imag_theme' ),
            ),
        )
...
...
```php
add_action( 'cmb2_render_address', 'cmb2_render_address_field', 10, 5 );
/**
 * Render Address Field
 */
function cmb2_render_address_field( $field_object, $value, $object_id, $object_type, $field_type_object ) {
...
> **Update: This field is now [in CMB2 core](https://github.com/WebDevStudios/CMB2/blob/master/example-functions.php#L64), so consider this tutorial as an example.**
...
```php
add_action( 'cmb2_render_text_url', 'jt_cmb_render_text_url', 10, 5 );
/**
